...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 frame_size = CalcItemSize(size_arg, GetContentRegionAvail().x / 2.f, (is_multiline ? g.FontSize * 8.0f : label_size.y) + style.FramePadding.y * 2.0f); // Arbitrary default of 8 lines high for multi-line
    const ImVec2 total_size = ImVec2(frame_size.x + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f), frame_size.y);

    ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + frame_size);

    if (flags & ImGuiInputTextFlags_RightBound)
        frame_bb = ImRect(ImVec2(window->WorkRect.Max.x - frame_size.x, window->DC.CursorPos.y), ImVec2(window->WorkRect.Max.x, window->DC.CursorPos.y + frame_size.y));
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_cursor)
        {
            static std::unordered_map<ImGuiID, CursorState> anim;
            CursorState& cursorState = anim[id];

            state->CursorAnim += io.DeltaTime * 1.3f;
            bool cursor_is_visible = (!g.IO.ConfigInputTextCursorBlink) || (state->CursorAnim <= 0.0f) || ImFmod(state->CursorAnim, 1.20f) <= 0.80f;
            ImVec2 cursor_screen_pos = ImTrunc(draw_pos + cursor_offset - draw_scroll);
            ImRect cursor_screen_rect(cursor_screen_pos.x, cursor_screen_pos.y - g.FontSize + 0.5f, cursor_screen_pos.x + 1.0f, cursor_screen_pos.y - 1.5f);
            cursorState.alpha = ImLerp(cursorState.alpha, cursor_is_visible ? 1.f : 0.f, g.IO.DeltaTime * 20);
            cursorState.sizeModifier = ImLerp(cursorState.sizeModifier, cursor_screen_rect.Min.x, g.IO.DeltaTime * 20);

            auto color = GetStyleColorVec4(ImGuiCol_Text);
            color.w = cursorState.alpha;

            if (cursor_screen_rect.Overlaps(clip_rect))
                draw_window->DrawList->AddLine(ImVec2(cursorState.sizeModifier, cursor_screen_rect.Min.y), ImVec2(cursorState.sizeModifier, cursor_screen_rect.GetBL().y), GetColorU32(color));
    ...
    if (label_size.x > 0)
        RenderText(ImVec2(window->DC.CursorPos.x, frame_bb.GetCenter().y - label_size.y / 2), label);
